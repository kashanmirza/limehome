...
export class BookingsService {
    constructor(@Inject(BOOKINGS_REPOSITORY) private readonly bookingsRepository: typeof Bookings) { }
    ...
    async getBookingsByHotelId(hotelId: number): Promise<{ rows: Bookings[]; count: number; }> {
        return await this.bookingsRepository.findAndCountAll<Bookings>({ where: { hotelId } });
    }
}
